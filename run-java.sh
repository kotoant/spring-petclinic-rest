...
if [[ "${SERVER:=tomcat}" =~ ^(jetty|netty|tomcat|undertow)$ ]]; then
  RUN_JAVA_OPTIONS="${JAVA_OPTIONS:-} -Dserver.port=${PORT:-9966} -Dspring.profiles.active=${PROFILE:=webmvc-jdbc} -XX:+PrintFlagsFinal --enable-preview -XX:+ExitOnOutOfMemoryError"
